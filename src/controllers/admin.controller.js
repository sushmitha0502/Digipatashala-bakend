...
const approveCourse = asyncHandler(async(req,res)=>{
    ...
    const toApprove = req.body.Isapproved
    const isCourseApproved = toApprove === true || toApprove === "approved"

    if (isCourseApproved) {
        const theCourse = await course.findOneAndUpdate({_id: courseID}, {$set: {isapproved: true}},  { new: true })

        if(!theCourse){
            throw new ApiError(400,"failed to approve course")
        }
...
const getRecordedClassesForAdmin = asyncHandler(async (req, res) => {
    ...
    const coursesWithRecorded = await course
        .find({ "recordedClasses.0": { $exists: true } })
        .select("coursename recordedClasses enrolledteacher")
        .populate("enrolledteacher", "Firstname Lastname Email");

    const recordedClasses = coursesWithRecorded.flatMap((courseItem) =>
        (courseItem.recordedClasses || []).map((item) => ({
            courseId: courseItem._id,
            coursename: courseItem.coursename,
            teacherName: `${courseItem.enrolledteacher?.Firstname || ""} ${courseItem.enrolledteacher?.Lastname || ""}`,
            teacherEmail: courseItem.enrolledteacher?.Email,
            title: item.title,
...
const approveRecordedClass = asyncHandler(async (req, res) => {
    const { adminID, courseID, recordID } = req.params;
    ...
    const recordItem = targetCourse.recordedClasses.id(recordID);

    if (!recordItem) {
        throw new ApiError(404, "Recorded class not found");
    }

    recordItem.approvedByAdmin = true;
    await targetCourse.save();

    return res
        .status(200)
        .json(new ApiResponse(200, { record: recordItem }, "recorded class approved"));
});
...
const getAdminUsers = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, { students, teachers }, "admin users fetched"));
});
...
const getAdminQuizzes = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, quizzes, "admin quizzes fetched"));
});
...
const updateStudentByAdmin = asyncHandler(async (req, res) => {
    ...
    if (updatedStudent.Studentdetails) {
        await studentdocs.findByIdAndUpdate(
            updatedStudent.Studentdetails,
            {
                ...(Phone !== undefined ? { Phone } : {}),
                ...(Address !== undefined ? { Address } : {}),
                ...(Highesteducation !== undefined ? { Highesteducation } : {}),
                ...(SecondarySchool !== undefined ? { SecondarySchool } : {}),
                ...(HigherSchool !== undefined ? { HigherSchool } : {}),
                ...(SecondaryMarks !== undefined ? { SecondaryMarks } : {}),
                ...(HigherMarks !== undefined ? { HigherMarks } : {}),
            },
            { new: true }
        );
    }
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, fullStudent, "student updated by admin"));
});

const updateTeacherByAdmin = asyncHandler(async (req, res) => {
    ...
    if (updatedTeacher.Teacherdetails) {
        await Teacherdocs.findByIdAndUpdate(
            updatedTeacher.Teacherdetails,
            {
                ...
                ...(SecondarySchool !== undefined ? { SecondarySchool } : {}),
                ...(HigherSchool !== undefined ? { HigherSchool } : {}),
                ...(UGcollege !== undefined ? { UGcollege } : {}),
                ...(PGcollege !== undefined ? { PGcollege } : {}),
                ...(SecondaryMarks !== undefined ? { SecondaryMarks } : {}),
                ...(HigherMarks !== undefined ? { HigherMarks } : {}),
                ...(UGmarks !== undefined ? { UGmarks } : {}),
                ...(PGmarks !== undefined ? { PGmarks } : {}),
            },
            { new: true }
        );
    }

    const fullTeacher = await Teacher.findById(teacherID)
        .select("-Password -Refreshtoken")
        .populate("Teacherdetails");

    return res
        .status(200)
        .json(new ApiResponse(200, fullTeacher, "teacher updated by admin"));
});
...
export {
    ...
    toapproveCourse,
    approveCourse,
    getRecordedClassesForAdmin,
    approveRecordedClass,
    getAdminUsers,
    getAdminQuizzes,
